...
export const WidgetGeneratorCard: React.FC<WidgetGeneratorCardProps> = ({ config, products, onChange }) => {
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
      ...
      <div>
        ...
        >
          {products.map(p => (
            <option key={p.id} value={p.id}>{p.name} ({p.price_monthly ? `$${p.price_monthly}/mo` : 'Free'})</option>
          ))}
        </select>
      </div>
