...
func TestAuthUsecase_Login(t *testing.T) {
	...
	// 1. Successful Login
	loginRes, err := uc.Login(ctx, auth.LoginDTO{
		Email:    "jane@example.com",
		Password: "CorrectPassword123",
	})
	if err != nil {
	...
	// 2. Wrong Password
	_, err = uc.Login(ctx, auth.LoginDTO{
		Email:    "jane@example.com",
...
func TestAuthUsecase_Profile(t *testing.T) {
	...
	// 2. Update Profile
	updateRes, err := uc.UpdateProfile(ctx, regRes.Client.ID, auth.UpdateProfileDTO{
		FirstName: "Alicia",
		LastName:  "Keys",
		City:      "Jakarta",
		Country:   "ID",
