...
func TestGenerateAndValidateToken(t *testing.T) {
	...
	tokenStr, err := GenerateToken(secret, clientID, email, role, 1*time.Hour)
	if err != nil {
...
func TestValidateToken_InvalidSecret(t *testing.T) {
	tokenStr, _ := GenerateToken("secret-1", 1, "a@b.com", "client", 1*time.Hour)

	_, err := ValidateToken("wrong-secret-key", tokenStr)
	if err == nil {
...
func TestValidateToken_ExpiredToken(t *testing.T) {
	...
	tokenStr, err := GenerateToken(secret, 1, "a@b.com", "client", -10*time.Second)
	if err != nil {
