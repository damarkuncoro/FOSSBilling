...
func InitHandlers(services *Services, repos *Repositories) *AppHandlers {
	return &AppHandlers{
		...
		ClientDeposit:      client.NewDepositHandler(services.Invoice, services.Payment),
		...
		AdminKB:            admin.NewKBHandler(services.KB),
		...
		AdminCatalog:       admin.NewCatalogHandler(services.Product, services.Server, repos.Catalog),
		AdminBilling:       admin.NewBillingModuleHandler(services.Stats, services.Tax, repos.Promo),
		AdminSystem:        admin.NewSystemModuleHandler(services.System, services.Page),
		AdminActivity:      admin.NewActivityHandler(services.Activity),
		AdminAntispam:      admin.NewAntispamHandler(services.Antispam),
		AdminFormbuilder:   admin.NewFormbuilderHandler(services.Formbuilder),
		AdminExtension:     admin.NewExtensionHandler(services.Extension),
		AdminRedirect:      admin.NewRedirectHandler(services.Redirect),
		AdminCookieConsent: admin.NewCookieConsentHandler(services.CookieConsent),
		AdminNotification:  admin.NewAdminNotificationHandler(services.AdminNotif),
		AdminTheme:         admin.NewThemeHandler(services.Theme),
		AdminSEO:           admin.NewSEOHandler(services.SEO),
		AdminWidget:        admin.NewWidgetHandler(services.Widget),
	}
}
